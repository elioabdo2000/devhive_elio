...
import { loginSchema, oauthPreAuthSchema } from "@/lib/validations/auth";
...
function LoginPageFallback() {
  return (
    <div className="w-full max-w-[380px] rounded-2xl border border-border bg-white p-8 shadow-lg">
...
function LoginPageInner() {
  ...
  // Gates the OAuth buttons: confirms email + terms acceptance with Zod
  // before initiating the redirect. Password is irrelevant here — Google
  // and GitHub handle those credentials, and this app never sees them.
  function validateOAuthGate() {
    const result = oauthPreAuthSchema.safeParse({ email, termsAccepted });
    if (!result.success) {
      setErrors((prev) => {
        const next = { ...prev };
        for (const issue of result.error.issues) {
  ...
  return (
    <div className="w-full max-w-[400px] rounded-2xl border border-border bg-white p-8 shadow-lg">
      ...
      {authError && (
        <div className="mb-4 rounded-lg bg-red-50 px-4 py-2 text-sm text-red-700">{authError}</div>
      )}
