...
function SignupPageInner() {
  ...
  // Gates the OAuth buttons on terms acceptance only, run before initiating
  // the redirect. Email and password are irrelevant here — Google/GitHub
  // supply the email themselves. (Previously this also required the page's
  // separate email field to be filled in first, which silently blocked
  // anyone who went straight for "Continue with Google/GitHub" without
  // touching that field — that was the actual OAuth login bug.)
  function validateOAuthGate() {
    const result = oauthPreAuthSchema.safeParse({ termsAccepted: form.termsAccepted });
    if (!result.success) {
      setErrors((prev) => ({ ...prev, termsAccepted: "You must accept the terms" }));
      showToast("error", "Please accept the Terms of Service to continue.");
      return false;
    }
    return true;
  }
    ...
    <div className="w-full max-w-[400px] rounded-2xl border border-border bg-paper-raised p-8 shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          />
          {errors.name && (
            <p id="signup-name-error" className="mt-1 text-xs text-danger">
              {errors.name}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.password && (
            <p id="signup-password-error" className="mt-1 text-xs text-danger">
              {errors.password}
            </p>
          )}
        </div>
        ...
        <div>
          ...
          {errors.confirmPassword && (
            <p id="signup-confirm-password-error" className="mt-1 text-xs text-danger">
              {errors.confirmPassword}
            </p>
          )}
        </div>
